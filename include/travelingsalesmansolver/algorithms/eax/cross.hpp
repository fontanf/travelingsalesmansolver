...
template <typename Distances>
TCross<Distances>::~TCross() {}

template <typename Distances>
void TCross<Distances>::setParents( const TIndi& tPa1, const TIndi& tPa2, int flagC[10], int numOfKids ){
...
template <typename Distances>
void TCross<Distances>::doIt( TIndi& tKid, TIndi& tPa2, int numOfKids, int flagP, int flagC[ 10 ], vector<vector<int>>& fEdgeFreq ){
	int Num;
	int jnum, centerAB;
	Distance gain;
	Distance BestGain;
...
template <typename Distances>
void TCross<Distances>::setABcycle( const TIndi& tPa1, const TIndi& tPa2, int flagC[ 10 ], int numOfKids ){
...
template <typename Distances>
void TCross<Distances>::formABcycle(){
	int j;
	int st_count;
	int edge_type;
	int st,ci, stock;
	...
	fC[ cem+1 ] = fC[ 1 ];
	diff = 0;
	for( j = 0; j < cem/2; ++j ) diff = diff + eval->distance(fC[2*j],fC[1+2*j]) - eval->distance(fC[1+2*j],fC[2+2*j]);
...
template <typename Distances>
void TCross<Distances>::swap(int &x, int &y){
	int s=x;
	x=y;
	y=s;
}
...
template <typename Distances>
void TCross<Distances>::changeSol( TIndi& tKid, int ABnum, int type ){
	int j;
	int cem, r1, r2, b1, b2;
...
template <typename Distances>
void TCross<Distances>::makeCompleteSol( TIndi& tKid ){
	int j, j1, j2;
	...
	RESTART:
			...
			for( near_num = 1; near_num <= nearMax; ++near_num ){
				...
				if( fCenterUnit[ c ] == 0 ){
					for( j1 = 0; j1 < 2; ++j1 ){
						...
						for( j2 = 0; j2 < 2; ++j2 ){
							d = tKid.fLink[ c ][ j2 ];
							diff = eval->distance(a,b) + eval->distance(c,d) - eval->distance(a,c) - eval->distance(b,d);
							if( diff > max_diff ){
								aa = a; bb = b; a1 = c; b1 = d;
								max_diff = diff;
							}
							diff = eval->distance(a,b) + eval->distance(d,c) -
								eval->distance(a,d) - eval->distance(b,c);
							if( diff > max_diff ){
								aa = a; bb = b; a1 = d; b1 = c;
								max_diff = diff;
							}
		...
		if( a1 == -1 && nearMax == 10 ){
			...
			max_diff = eval->distance(aa,bb) + eval->distance(a1,b1) - eval->distance(a,a1) - eval->distance(b,b1);
		}
...
template <typename Distances>
void TCross<Distances>::makeUnit(){
	int flag = 1;
...
template <typename Distances>
void TCross<Distances>::backToPa1( TIndi& tKid ){
	int aa, bb, a1, b1;
	int jnum;
...
template <typename Distances>
void TCross<Distances>::goToBest( TIndi& tKid ){
	int aa, bb, a1, b1;
	int jnum;
...
template <typename Distances>
void TCross<Distances>::incrementEdgeFreq(vector<vector<int>>& fEdgeFreq){
	int j, jnum, cem;
	int r1, r2, b1, b2;
	int aa, bb, a1;
...
template <typename Distances>
int TCross<Distances>::calAdpLoss(vector<vector<int>>& fEdgeFreq){
	int j, jnum, cem;
	int r1, r2, b1, b2;
	int aa, bb, a1;
	double DLoss;
...
template <typename Distances>
double TCross<Distances>::calEntLoss(vector<vector<int>>& fEdgeFreq){
	int j, jnum, cem;
	int r1, r2, b1, b2;
	int aa, bb, a1;
	double DLoss;
...
template <typename Distances>
void TCross<Distances>::setWeight( const TIndi& tPa1, const TIndi& tPa2 ){
	int cem;
	int r1, r2, v1, v2, v_p;
	int AB_num;
...
template <typename Distances>
int TCross<Distances>::calCNaive(){
	int count_C;
	int tt;
...
template <typename Distances>
void TCross<Distances>::searchEset( int centerAB ){
	int nIter, stagImp;
...
template <typename Distances>
void TCross<Distances>::addAB( int num ){
...
template <typename Distances>
void TCross<Distances>::deleteAB( int num ){
	fNumC -= fWeightC[ num ] - 2 * fWeightSR[ num ];
	fNumE -= fABcycle[ num ][ 0 ] / 2;
