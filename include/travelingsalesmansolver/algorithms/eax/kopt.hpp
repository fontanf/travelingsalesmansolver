...
template <typename Distances>
void TKopt<Distances>::setInvNearList(){
...
template <typename Distances>
void TKopt<Distances>::transIndiToTree( TIndi& indi ){
	int num, size, orient;
	fArray[1] = 0;
...
template <typename Distances>
void TKopt<Distances>::transTreeToIndi( TIndi& indi ){
	int t_p, t_n;
	for( int t = 0; t < fN; ++t ){
		t_p = this->getPrev( t );
		t_n = this->getNext( t );
...
template <typename Distances>
void TKopt<Distances>::doIt( TIndi& tIndi ){
...
BEGIN:
	...
	while(1){
		...
		for( int num1 = 1; num1 < 50; ++num1 ){
			...
			dis1 = eval->distance(fT[1],fT[2]) - eval->distance(fT[1],fT[4]);
			if( dis1 > 0 ){
				dis2 = dis1 + eval->distance(fT[3],fT[4]) - eval->distance(fT[3],fT[2]);
				if( dis2 > 0 ){
...
template <typename Distances>
int TKopt<Distances>::getNext( int t ){
	int t_n, seg, orient;
	seg = fSegCity[ t ];
	orient = fOrient[ seg ];
	t_n = fLink[ t ][ orient ];
...
template <typename Distances>
int TKopt<Distances>::getPrev( int t ){
	int t_p, seg, orient;
	seg = fSegCity[ t ];
	orient = fOrient[ seg ];
...
template <typename Distances>
void TKopt<Distances>::swap(int &x, int &y){
	int s=x;
	x=y;
	y=s;
}
...
template <typename Distances>
int TKopt<Distances>::turn( int &orient ){
	return 1-orient;
}

template <typename Distances>
void TKopt<Distances>::incrementImp( int flagRev ){
...
template <typename Distances>
void TKopt<Distances>::combineSeg( int segL, int segS ){
	int seg;
...
template <typename Distances>
void TKopt<Distances>::checkDetail(){
	int seg, seg_p, seg_n;
	int ord, ord_p, ord_n;
	int orient;
	int curr;
...
template <typename Distances>
void TKopt<Distances>::checkValid(){
...
template <typename Distances>
void TKopt<Distances>::makeRandSol( TIndi& indi ){
