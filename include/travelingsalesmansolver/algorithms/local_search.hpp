...
struct LocalSearchData
{
    ...
    /**
     * 'weight_rr[ab_number]' lists, sorted by increasing AB-cycle number, the
     * other AB-cycles sharing vertices with 'ab_number' along with the number
     * of shared vertices.
     */
    std::vector<std::vector<std::pair<int, int>>> weight_rr;
        ...
        int number_of_children)
{
    ...
    data.weight_rr.assign(number_of_vertices, {});

    data.weight_sr.resize(number_of_vertices);
    data.weight_c.resize(number_of_vertices);
        ...
        const Individual& parent2)
{
    ...
    for (int s1 = 0; s1 < data.number_of_ab_cycles; ++s1) {
        data.weight_c[s1] = 0;
        data.weight_rr[s1].clear();
    }

    // Number of vertices shared by each pair of distinct AB-cycles.
    std::vector<std::array<int, 2>> shared_vertices;
    for (VertexId vertex_id = 0; vertex_id < data.number_of_vertices; ++vertex_id) {
        int ab_number_1 = data.in_effect_node[vertex_id][0];
        int ab_number_2 = data.in_effect_node[vertex_id][1];
        ...
        if (ab_number_1 != ab_number_2) {
            ++data.weight_c[ab_number_1];
            ++data.weight_c[ab_number_2];
        }
    }
    std::sort(shared_vertices.begin(), shared_vertices.end());
    for (const std::array<int, 2>& pair: shared_vertices) {
        std::vector<std::pair<int, int>>& weights = data.weight_rr[pair[0]];
        ...
        int num)
{
    ...
    for (const std::pair<int, int>& weight: data.weight_rr[num])
        data.weight_sr[weight.first] += weight.second;
}
        ...
        int flag_p)
{
    ...
    for (int j = 0; j < number_of_candidates; ++j) {
        ...
        if (data.eset_strategy == 1) { //Single-AB
            ...
            for (const std::pair<int, int>& weight: data.weight_rr[center_ab]) {
                int s = weight.first;
