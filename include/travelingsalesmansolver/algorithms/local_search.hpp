...
{
    /** Create a solution with no vertices. */
    LocalSearchSolution() { }

    /** Create a solution for a given number of vertices. */
    LocalSearchSolution(VertexId number_of_vertices):
        neighbors(number_of_vertices) { }

    /** Checks if two solutions represent the same tour. */
    bool operator==(
            const LocalSearchSolution& solution) const
    {
        if (neighbors.size() != solution.neighbors.size())
            return false;
        ...
        for (std::size_t i = 0; i < neighbors.size(); ++i) {
            ...
            if (solution.neighbors[current_vertex_id][0] != next_vertex_id && solution.neighbors[current_vertex_id][1] != next_vertex_id)
                return false;
...
/**
 * A tour represented as a two-level doubly-linked list: the tour is split into
 * segments, each being a doubly-linked path of vertices, and the segments are
 * themselves linked to each other. It allows reversing a path (a 2-opt move)
 * in time roughly proportional to the square root of the number of vertices.
 */
struct TreeSolution
{
    /** Create a tree solution for a given number of vertices. */
    TreeSolution(VertexId number_of_vertices):
    ...
    /** 'neighbors[vertex_id]' holds the two vertices adjacent to 'vertex_id' within its segment ('-1' at a segment end). */
    std::vector<std::array<VertexId, 2>> neighbors;
    ...
    /** Scratch array used to build the tree in 'solution_to_tree()'. */
...
struct LocalSearchData
{
    ...
    /** Instance, used to build a 'Solution' every time a new best solution is found. */
    const Instance& instance;

    /** Algorithm parameters (used to check 'parameters.timer.needs_to_end()'). */
    const LocalSearchParameters& parameters;

    /** Used to cheaply check whether a candidate solution actually improves on the best solution found so far, before converting it to a 'Solution'. */
    const Output& output;

    /** Used to report every new best solution found, as soon as it is found. */
    AlgorithmFormatter& algorithm_formatter;
    ...
    /** Tree representation of the solution being locally optimized. */
    TreeSolution tree;
    ...
    /** Current population. */
    std::vector<LocalSearchSolution> population;

    /** Best solution found so far. */
    LocalSearchSolution best_solution;
    ...
    /** Tour length of the best solution in the population. */
    ...
    /** Number of consecutive generations without an improvement to 'best_solution'. */
        ...
        int number_of_children)
{
    ...
    // even though each solution position is a valid index into a tour of
    ...
    data.population = std::vector<LocalSearchSolution>(population_size, LocalSearchSolution(number_of_vertices));
    data.best_solution = LocalSearchSolution(number_of_vertices);
    data.edge_frequency = EdgeFrequencies(number_of_vertices);
    data.index_for_mating.resize(population_size + 1);
}

/** Compute and store the length of an solution's tour. */
template <typename Distances>
void evaluate(
        ...
        LocalSearchSolution& solution)
{
    ...
    for (VertexId vertex_id = 0; vertex_id < data.number_of_vertices; ++vertex_id)
        twice_length += data.distances.distance(vertex_id, solution.neighbors[vertex_id][0]) + data.distances.distance(vertex_id, solution.neighbors[vertex_id][1]);
    solution.length = twice_length / 2;
}

/** Build a 'Solution' from the tour of a 'LocalSearchSolution'. */
template <typename Distances>
        ...
        const LocalSearchSolution& local_search_solution)
{
    ...
    for (int count = 0; count < data.number_of_vertices; ++count) {
        VertexId next_vertex_id = (local_search_solution.neighbors[current_vertex_id][0] == previous_vertex_id)?
            local_search_solution.neighbors[current_vertex_id][1]:
            local_search_solution.neighbors[current_vertex_id][0];
        previous_vertex_id = current_vertex_id;
        current_vertex_id = next_vertex_id;
...
inline void solution_to_tree(
        TreeSolution& tree,
        const LocalSearchSolution& solution)
{
    tree.buffer[1] = 0;
    for (int i = 2; i <= tree.number_of_vertices; ++i)
        tree.buffer[i] = solution.neighbors[tree.buffer[i - 1]][1];
    tree.buffer[0] = tree.buffer[tree.number_of_vertices];
    tree.buffer[tree.number_of_vertices + 1] = tree.buffer[1];

    int position = 1;
    tree.number_of_segments = 0;
    while (true) {
        int orientation = 1;
        int size = 0;
        tree.segment_orientation[tree.number_of_segments] = orientation;
        tree.segment_order[tree.number_of_segments] = tree.number_of_segments;

        tree.neighbors[tree.buffer[position]][0] = -1;
        tree.neighbors[tree.buffer[position]][1] = tree.buffer[position + 1];
        tree.vertex_order[tree.buffer[position]] = size;
        tree.vertex_segment[tree.buffer[position]] = tree.number_of_segments;
        tree.segment_endpoints[tree.number_of_segments][opposite_orientation(orientation)] = tree.buffer[position];
        ++position;
        ++size;
        for (int i = 0; i < (int)std::sqrt(tree.number_of_vertices * 1.0) - 1; ++i) {
            if (position == tree.number_of_vertices)
                break;
            tree.neighbors[tree.buffer[position]][0] = tree.buffer[position - 1];
            tree.neighbors[tree.buffer[position]][1] = tree.buffer[position + 1];
            tree.vertex_order[tree.buffer[position]] = size;
            tree.vertex_segment[tree.buffer[position]] = tree.number_of_segments;
            ++position;
        ...
        if (position == tree.number_of_vertices - 1) {
            tree.neighbors[tree.buffer[position]][0] = tree.buffer[position - 1];
            tree.neighbors[tree.buffer[position]][1] = tree.buffer[position + 1];
            tree.vertex_order[tree.buffer[position]] = size;
            tree.vertex_segment[tree.buffer[position]] = tree.number_of_segments;
        ...
        tree.neighbors[tree.buffer[position]][0] = tree.buffer[position - 1];
        tree.neighbors[tree.buffer[position]][1] = -1;
        tree.vertex_order[tree.buffer[position]] = size;
        tree.vertex_segment[tree.buffer[position]] = tree.number_of_segments;
        tree.segment_endpoints[tree.number_of_segments][orientation] = tree.buffer[position];
        ++position;
        ++size;
        tree.segment_size[tree.number_of_segments] = size;
        ++tree.number_of_segments;
        if (position == tree.number_of_vertices + 1)
            break;
    }
    for (int s = 1; s < tree.number_of_segments - 1; ++s) {
        tree.segment_neighbors[s][0] = s - 1;
        tree.segment_neighbors[s][1] = s + 1;
    }
    tree.segment_neighbors[0][0] = tree.number_of_segments - 1;
    tree.segment_neighbors[0][1] = 1;
    tree.segment_neighbors[tree.number_of_segments - 1][0] = tree.number_of_segments - 2;
    tree.segment_neighbors[tree.number_of_segments - 1][1] = 0;
    tree.fixed_number_of_segments = tree.number_of_segments;
}

/** Vertex immediately after 'vertex_id' in the tour. */
inline VertexId next_vertex(
        const TreeSolution& tree,
        VertexId vertex_id)
{
    int segment_id = tree.vertex_segment[vertex_id];
    int orientation = tree.segment_orientation[segment_id];
    VertexId next_vertex_id = tree.neighbors[vertex_id][orientation];
    if (next_vertex_id == -1) {
        segment_id = tree.segment_neighbors[segment_id][orientation];
        orientation = opposite_orientation(tree.segment_orientation[segment_id]);
        next_vertex_id = tree.segment_endpoints[segment_id][orientation];
    }
    return next_vertex_id;
}

/** Vertex immediately before 'vertex_id' in the tour. */
inline VertexId previous_vertex(
        const TreeSolution& tree,
        VertexId vertex_id)
{
    int segment_id = tree.vertex_segment[vertex_id];
    int orientation = tree.segment_orientation[segment_id];
    VertexId previous_vertex_id = tree.neighbors[vertex_id][opposite_orientation(orientation)];
    if (previous_vertex_id == -1) {
        segment_id = tree.segment_neighbors[segment_id][opposite_orientation(orientation)];
        orientation = tree.segment_orientation[segment_id];
        previous_vertex_id = tree.segment_endpoints[segment_id][orientation];
    }
    return previous_vertex_id;
}

/** Rebuild 'solution' from the tree representation. */
inline void tree_to_solution(
        const TreeSolution& tree,
        LocalSearchSolution& solution)
{
    for (VertexId vertex_id = 0; vertex_id < tree.number_of_vertices; ++vertex_id) {
        solution.neighbors[vertex_id][0] = previous_vertex(tree, vertex_id);
        solution.neighbors[vertex_id][1] = next_vertex(tree, vertex_id);
    }
}
        ...
        int segment_2)
{
    ...
    if (tree.segment_neighbors[segment_1][tree.segment_orientation[segment_1]] == segment_2) {
        tree.neighbors[tree.segment_endpoints[segment_1][tree.segment_orientation[segment_1]]][tree.segment_orientation[segment_1]] =
            tree.segment_endpoints[segment_2][opposite_orientation(tree.segment_orientation[segment_2])];
        tree.neighbors[tree.segment_endpoints[segment_2][opposite_orientation(tree.segment_orientation[segment_2])]][opposite_orientation(tree.segment_orientation[segment_2])] =
            tree.segment_endpoints[segment_1][tree.segment_orientation[segment_1]];
        next_order = tree.vertex_order[tree.segment_endpoints[segment_1][tree.segment_orientation[segment_1]]];

        tree.segment_endpoints[segment_1][tree.segment_orientation[segment_1]] = tree.segment_endpoints[segment_2][tree.segment_orientation[segment_2]];
        tree.segment_neighbors[segment_1][tree.segment_orientation[segment_1]] = tree.segment_neighbors[segment_2][tree.segment_orientation[segment_2]];
        int segment_id = tree.segment_neighbors[segment_2][tree.segment_orientation[segment_2]];
        tree.segment_neighbors[segment_id][opposite_orientation(tree.segment_orientation[segment_id])] = segment_1;

        moved_start_vertex_id = tree.segment_endpoints[segment_2][opposite_orientation(tree.segment_orientation[segment_2])];
        moved_end_vertex_id = tree.segment_endpoints[segment_2][tree.segment_orientation[segment_2]];
        direction = tree.segment_orientation[segment_2];

        increment = (tree.segment_orientation[segment_1] == 1) ? 1 : -1;
    } else if (tree.segment_neighbors[segment_1][opposite_orientation(tree.segment_orientation[segment_1])] == segment_2) {
        tree.neighbors[tree.segment_endpoints[segment_1][opposite_orientation(tree.segment_orientation[segment_1])]][opposite_orientation(tree.segment_orientation[segment_1])] =
            tree.segment_endpoints[segment_2][tree.segment_orientation[segment_2]];
        tree.neighbors[tree.segment_endpoints[segment_2][tree.segment_orientation[segment_2]]][tree.segment_orientation[segment_2]] =
            tree.segment_endpoints[segment_1][opposite_orientation(tree.segment_orientation[segment_1])];
        next_order = tree.vertex_order[tree.segment_endpoints[segment_1][opposite_orientation(tree.segment_orientation[segment_1])]];

        tree.segment_endpoints[segment_1][opposite_orientation(tree.segment_orientation[segment_1])] = tree.segment_endpoints[segment_2][opposite_orientation(tree.segment_orientation[segment_2])];
        tree.segment_neighbors[segment_1][opposite_orientation(tree.segment_orientation[segment_1])] = tree.segment_neighbors[segment_2][opposite_orientation(tree.segment_orientation[segment_2])];
        int segment_id = tree.segment_neighbors[segment_2][opposite_orientation(tree.segment_orientation[segment_2])];
        tree.segment_neighbors[segment_id][tree.segment_orientation[segment_id]] = segment_1;

        moved_start_vertex_id = tree.segment_endpoints[segment_2][tree.segment_orientation[segment_2]];
        moved_end_vertex_id = tree.segment_endpoints[segment_2][opposite_orientation(tree.segment_orientation[segment_2])];
        direction = opposite_orientation(tree.segment_orientation[segment_2]);
    ...
    while (true) {
        tree.vertex_segment[current_vertex_id] = segment_1;
        tree.vertex_order[current_vertex_id] = next_order;

        VertexId next_vertex_id = tree.neighbors[current_vertex_id][direction];
        if (tree.segment_orientation[segment_1] != tree.segment_orientation[segment_2])
            std::swap(tree.neighbors[current_vertex_id][0], tree.neighbors[current_vertex_id][1]);
    ...
    tree.segment_size[segment_1] += tree.segment_size[segment_2];
    --tree.number_of_segments;
}
        ...
        bool reversed)
{
    ...
    int segment_path_1_start = tree.vertex_segment[path_1_start];
    int segment_order_path_1_start = tree.segment_order[segment_path_1_start];
    int orientation_path_1_start = tree.segment_orientation[segment_path_1_start];
    int segment_path_1_end = tree.vertex_segment[path_1_end];
    int segment_order_path_1_end = tree.segment_order[segment_path_1_end];
    int orientation_path_1_end = tree.segment_orientation[segment_path_1_end];
    int segment_path_2_start = tree.vertex_segment[path_2_start];
    int segment_order_path_2_start = tree.segment_order[segment_path_2_start];
    int orientation_path_2_start = tree.segment_orientation[segment_path_2_start];
    int segment_path_2_end = tree.vertex_segment[path_2_end];
    int segment_order_path_2_end = tree.segment_order[segment_path_2_end];
    int orientation_path_2_end = tree.segment_orientation[segment_path_2_end];
    ...
    if ((segment_path_1_start == segment_path_1_end) && (segment_path_1_start == segment_path_2_start) && (segment_path_1_start == segment_path_2_end)) {
        if ((tree.segment_orientation[segment_path_1_start] == 1 && (tree.vertex_order[path_1_start] > tree.vertex_order[path_1_end])) ||
            (tree.segment_orientation[segment_path_1_start] == 0 && (tree.vertex_order[path_1_start] < tree.vertex_order[path_1_end]))) {
        ...
        int next_order = tree.vertex_order[path_1_end];
        while (true) {
            std::swap(tree.neighbors[current_vertex_id][0], tree.neighbors[current_vertex_id][1]);
            tree.vertex_order[current_vertex_id] = next_order;
            if (current_vertex_id == path_1_end)
                break;
            current_vertex_id = tree.neighbors[current_vertex_id][opposite_orientation(orientation_path_1_start)];
        ...
        tree.neighbors[path_2_end][orientation_path_1_start] = path_1_end;
        tree.neighbors[path_2_start][opposite_orientation(orientation_path_1_start)] = path_1_start;
        tree.neighbors[path_1_start][orientation_path_1_start] = path_2_start;
        tree.neighbors[path_1_end][opposite_orientation(orientation_path_1_start)] = path_2_end;
    ...
    int number_of_segments_path_1 = (segment_order_path_1_end >= segment_order_path_1_start) ?
        ...
        segment_order_path_1_end - segment_order_path_1_start + 1 + tree.number_of_segments;
    int number_of_segments_path_2 = (segment_order_path_2_end >= segment_order_path_2_start) ?
        ...
        segment_order_path_2_end - segment_order_path_2_start + 1 + tree.number_of_segments;
    ...
    int path_2_end_is_segment_end = (tree.neighbors[path_2_end][orientation_path_2_end] == -1) ? 1 : 0;
    int path_2_start_is_segment_end = (tree.neighbors[path_2_start][opposite_orientation(orientation_path_2_start)] == -1) ? 1 : 0;

    int length_of_segment_at_path_1_start = std::abs(tree.vertex_order[path_2_end] - tree.vertex_order[tree.segment_endpoints[segment_path_2_end][orientation_path_2_end]]);
    int length_of_segment_at_path_1_end = std::abs(tree.vertex_order[path_2_start] - tree.vertex_order[tree.segment_endpoints[segment_path_2_start][opposite_orientation(orientation_path_2_start)]]);

    ///////////////////// Type2 /////////////////
    if (segment_path_1_start == segment_path_1_end) {
        if (path_2_end_is_segment_end == 1 && path_2_start_is_segment_end == 1) {
            orientation_path_1_start = opposite_orientation(tree.segment_orientation[segment_path_1_start]);
            tree.segment_orientation[segment_path_1_start] = orientation_path_1_start;
            tree.segment_endpoints[segment_path_1_start][orientation_path_1_start] = path_1_start;
            tree.segment_endpoints[segment_path_1_start][opposite_orientation(orientation_path_1_start)] = path_1_end;
            tree.segment_neighbors[segment_path_1_start][orientation_path_1_start] = segment_path_2_start;
            tree.segment_neighbors[segment_path_1_start][opposite_orientation(orientation_path_1_start)] = segment_path_2_end;
            return;
        }
        if (path_2_end_is_segment_end == 0 && path_2_start_is_segment_end == 1) {
            VertexId current_vertex_id = path_1_end;
            int next_order = tree.vertex_order[path_1_start];
            while (true) {
                std::swap(tree.neighbors[current_vertex_id][0], tree.neighbors[current_vertex_id][1]);
                tree.vertex_order[current_vertex_id] = next_order;
                if (current_vertex_id == path_1_start)
                    break;
                current_vertex_id = tree.neighbors[current_vertex_id][orientation_path_2_end];
                if (orientation_path_2_end == 0)
                    --next_order;
                else
                    ++next_order;
            }
            tree.neighbors[path_2_end][orientation_path_2_end] = path_1_end;
            tree.neighbors[path_1_start][orientation_path_2_end] = -1;
            tree.neighbors[path_1_end][opposite_orientation(orientation_path_2_end)] = path_2_end;
            tree.segment_endpoints[segment_path_2_end][orientation_path_2_end] = path_1_start;
            return;
        }
        if (path_2_end_is_segment_end == 1 && path_2_start_is_segment_end == 0) {
            ...
            while (true) {
                ...
                if (current_vertex_id == path_1_end)
                    break;
                current_vertex_id = tree.neighbors[current_vertex_id][opposite_orientation(orientation_path_2_start)];
                if (orientation_path_2_start == 0)
                    ++next_order;
                else
                    --next_order;
            }
            tree.neighbors[path_2_start][opposite_orientation(orientation_path_2_start)] = path_1_start;
            tree.neighbors[path_1_end][opposite_orientation(orientation_path_2_start)] = -1;
            tree.neighbors[path_1_start][orientation_path_2_start] = path_2_start;
            tree.segment_endpoints[segment_path_2_start][opposite_orientation(orientation_path_2_start)] = path_1_end;
            return;
        }
    }
    ...
    if (path_2_end_is_segment_end == 1) {
        tree.segment_neighbors[segment_path_1_start][opposite_orientation(orientation_path_1_start)] = segment_path_2_start;
    } else {
        segment_path_1_start = tree.number_of_segments++;
        orientation_path_1_start = orientation_path_2_end;
        tree.neighbors[path_1_start][opposite_orientation(orientation_path_1_start)] = -1;
        tree.neighbors[tree.segment_endpoints[segment_path_2_end][orientation_path_2_end]][orientation_path_1_start] = -1;
        tree.segment_orientation[segment_path_1_start] = orientation_path_1_start;
        tree.segment_size[segment_path_1_start] = length_of_segment_at_path_1_start;
        tree.segment_endpoints[segment_path_1_start][opposite_orientation(orientation_path_1_start)] = path_1_start;
        tree.segment_endpoints[segment_path_1_start][orientation_path_1_start] = tree.segment_endpoints[segment_path_2_end][orientation_path_2_end];
        tree.segment_neighbors[segment_path_1_start][opposite_orientation(orientation_path_1_start)] = segment_path_2_start;
        tree.segment_neighbors[segment_path_1_start][orientation_path_1_start] = tree.segment_neighbors[segment_path_2_end][orientation_path_2_end];
        int segment_id = tree.segment_neighbors[segment_path_2_end][orientation_path_2_end];
        tree.segment_neighbors[segment_id][opposite_orientation(tree.segment_orientation[segment_id])] = segment_path_1_start;
    }

    if (path_2_start_is_segment_end == 1) {
        tree.segment_neighbors[segment_path_1_end][orientation_path_1_end] = segment_path_2_end;
    } else {
        segment_path_1_end = tree.number_of_segments++;
        orientation_path_1_end = orientation_path_2_start;
        tree.neighbors[path_1_end][orientation_path_1_end] = -1;
        tree.neighbors[tree.segment_endpoints[segment_path_2_start][opposite_orientation(orientation_path_2_start)]][opposite_orientation(orientation_path_1_end)] = -1;
        tree.segment_orientation[segment_path_1_end] = orientation_path_1_end;
        tree.segment_size[segment_path_1_end] = length_of_segment_at_path_1_end;
        tree.segment_endpoints[segment_path_1_end][orientation_path_1_end] = path_1_end;
        tree.segment_endpoints[segment_path_1_end][opposite_orientation(orientation_path_1_end)] = tree.segment_endpoints[segment_path_2_start][opposite_orientation(orientation_path_2_start)];
        tree.segment_neighbors[segment_path_1_end][orientation_path_1_end] = segment_path_2_end;
        tree.segment_neighbors[segment_path_1_end][opposite_orientation(orientation_path_1_end)] = tree.segment_neighbors[segment_path_2_start][opposite_orientation(orientation_path_2_start)];
        int segment_id = tree.segment_neighbors[segment_path_2_start][opposite_orientation(orientation_path_2_start)];
        tree.segment_neighbors[segment_id][tree.segment_orientation[segment_id]] = segment_path_1_end;
    }

    tree.neighbors[path_2_end][orientation_path_2_end] = -1;
    tree.segment_size[segment_path_2_end] -= length_of_segment_at_path_1_start;
    tree.segment_endpoints[segment_path_2_end][orientation_path_2_end] = path_2_end;
    tree.segment_neighbors[segment_path_2_end][orientation_path_2_end] = segment_path_1_end;
    tree.neighbors[path_2_start][opposite_orientation(orientation_path_2_start)] = -1;
    tree.segment_size[segment_path_2_start] -= length_of_segment_at_path_1_end;
    tree.segment_endpoints[segment_path_2_start][opposite_orientation(orientation_path_2_start)] = path_2_start;
    tree.segment_neighbors[segment_path_2_start][opposite_orientation(orientation_path_2_start)] = segment_path_1_start;

    {
        int segment_id = segment_path_1_end;
        while (true) {
            tree.segment_orientation[segment_id] = opposite_orientation(tree.segment_orientation[segment_id]);
            if (segment_id == segment_path_1_start)
                break;
            segment_id = tree.segment_neighbors[segment_id][tree.segment_orientation[segment_id]];
        }
    }

    if (tree.segment_size[segment_path_2_end] < length_of_segment_at_path_1_start) {
        int segment_id = tree.segment_neighbors[segment_path_2_end][opposite_orientation(tree.segment_orientation[segment_path_2_end])];
        tree.segment_neighbors[segment_id][tree.segment_orientation[segment_id]] = segment_path_1_start;
        segment_id = tree.segment_neighbors[segment_path_2_end][tree.segment_orientation[segment_path_2_end]];
        tree.segment_neighbors[segment_id][opposite_orientation(tree.segment_orientation[segment_id])] = segment_path_1_start;
        segment_id = tree.segment_neighbors[segment_path_1_start][opposite_orientation(tree.segment_orientation[segment_path_1_start])];
        tree.segment_neighbors[segment_id][tree.segment_orientation[segment_id]] = segment_path_2_end;
        segment_id = tree.segment_neighbors[segment_path_1_start][tree.segment_orientation[segment_path_1_start]];
        tree.segment_neighbors[segment_id][opposite_orientation(tree.segment_orientation[segment_id])] = segment_path_2_end;

        std::swap(tree.segment_orientation[segment_path_2_end], tree.segment_orientation[segment_path_1_start]);
        std::swap(tree.segment_size[segment_path_2_end], tree.segment_size[segment_path_1_start]);
        std::swap(tree.segment_endpoints[segment_path_2_end][0], tree.segment_endpoints[segment_path_1_start][0]);
        std::swap(tree.segment_endpoints[segment_path_2_end][1], tree.segment_endpoints[segment_path_1_start][1]);
        std::swap(tree.segment_neighbors[segment_path_2_end][0], tree.segment_neighbors[segment_path_1_start][0]);
        std::swap(tree.segment_neighbors[segment_path_2_end][1], tree.segment_neighbors[segment_path_1_start][1]);
        std::swap(segment_path_2_end, segment_path_1_start);
    }

    if (tree.segment_size[segment_path_2_start] < length_of_segment_at_path_1_end) {
        int segment_id = tree.segment_neighbors[segment_path_2_start][opposite_orientation(tree.segment_orientation[segment_path_2_start])];
        tree.segment_neighbors[segment_id][tree.segment_orientation[segment_id]] = segment_path_1_end;
        segment_id = tree.segment_neighbors[segment_path_2_start][tree.segment_orientation[segment_path_2_start]];
        tree.segment_neighbors[segment_id][opposite_orientation(tree.segment_orientation[segment_id])] = segment_path_1_end;
        segment_id = tree.segment_neighbors[segment_path_1_end][opposite_orientation(tree.segment_orientation[segment_path_1_end])];
        tree.segment_neighbors[segment_id][tree.segment_orientation[segment_id]] = segment_path_2_start;
        segment_id = tree.segment_neighbors[segment_path_1_end][tree.segment_orientation[segment_path_1_end]];
        tree.segment_neighbors[segment_id][opposite_orientation(tree.segment_orientation[segment_id])] = segment_path_2_start;

        std::swap(tree.segment_orientation[segment_path_2_start], tree.segment_orientation[segment_path_1_end]);
        std::swap(tree.segment_size[segment_path_2_start], tree.segment_size[segment_path_1_end]);
        std::swap(tree.segment_endpoints[segment_path_2_start][0], tree.segment_endpoints[segment_path_1_end][0]);
        std::swap(tree.segment_endpoints[segment_path_2_start][1], tree.segment_endpoints[segment_path_1_end][1]);
        std::swap(tree.segment_neighbors[segment_path_2_start][0], tree.segment_neighbors[segment_path_1_end][0]);
        std::swap(tree.segment_neighbors[segment_path_2_start][1], tree.segment_neighbors[segment_path_1_end][1]);
        std::swap(segment_path_2_start, segment_path_1_end);
    }

    while (tree.number_of_segments > tree.fixed_number_of_segments) {
        if (tree.segment_size[tree.segment_neighbors[tree.number_of_segments - 1][0]] <
                tree.segment_size[tree.segment_neighbors[tree.number_of_segments - 1][1]])
            merge_segments(tree, tree.segment_neighbors[tree.number_of_segments - 1][0], tree.number_of_segments - 1);
        else
            merge_segments(tree, tree.segment_neighbors[tree.number_of_segments - 1][1], tree.number_of_segments - 1);
    }
    int order = 0;
    int segment_id = 0;
    while (true) {
        tree.segment_order[segment_id] = order;
        ++order;
        segment_id = tree.segment_neighbors[segment_id][tree.segment_orientation[segment_id]];
        if (segment_id == 0)
            break;
            ...
            if (data.active[t[1]] != 0) {
                ...
                for (bool reversed: {false, true}) {
                    if (move_applied)
                        break;
                    t[2] = (!reversed)?
                        previous_vertex(data.tree, t[1]):
                        next_vertex(data.tree, t[1]);
                    for (int near_index = 1; near_index < data.number_of_near_vertices; ++near_index) {
                        t[4] = data.near_vertices[t[1]][near_index];
                        t[3] = (!reversed)?
                            previous_vertex(data.tree, t[4]):
                            next_vertex(data.tree, t[4]);
                        ...
                        if (gain > 0) {
                            apply_move(data.tree, t, reversed);
                            for (int a = 1; a <= 4; ++a)
...
/** Run the local search on 'solution'. */
template <typename Distances>
void run_kopt(
        LocalSearchData<Distances>& data,
        LocalSearchSolution& solution)
{
    solution_to_tree(data.tree, solution);
    optimize(data);
    tree_to_solution(data.tree, solution);
    evaluate(data, solution);
}

/** Set 'solution' to a random tour and run the local search on it. */
template <typename Distances>
        ...
        LocalSearchSolution& solution)
{
    ...
    for (int j = 1; j < data.number_of_vertices - 1; ++j) {
        solution.neighbors[gene[j]][0] = gene[j - 1];
        solution.neighbors[gene[j]][1] = gene[j + 1];
    }
    solution.neighbors[gene[0]][0] = gene[data.number_of_vertices - 1];
    solution.neighbors[gene[0]][1] = gene[1];
    solution.neighbors[gene[data.number_of_vertices - 1]][0] = gene[data.number_of_vertices - 2];
    solution.neighbors[gene[data.number_of_vertices - 1]][1] = gene[0];
...
void set_ab_cycle(
        LocalSearchData<Distances>& data,
        const LocalSearchSolution& parent1,
        const LocalSearchSolution& parent2,
        int number_of_kids)
{
...
/** Update 'average_value'/'best_value'/'best_solution' from the current population. */
template <typename Distances>
void set_average_best(
        LocalSearchData<Distances>& data)
{
    Distance stock_best = data.best_solution.length;
    ...
    data.best_solution = data.population[best_index];
    data.average_value /= (double)data.population_size;
    if (data.best_solution.length < stock_best) {
        data.stagnation_count = 0;
    } else {
...
{
    for (int i = 0; i < data.population_size; ++i) {
        // Always build at least the first solution regardless of the time
        ...
        if (!data.output.solution.feasible()
            ...
            data.algorithm_formatter.update_solution(
                    ...
                    "initial solution " + std::to_string(i));
        }
    }
}
...
/** Cross the pair of solutions at 'index_for_mating[s]'/'[s + 1]'. */
template <typename Distances>
void generate_kids(
        LocalSearchData<Distances>& data,
        int s)
        ...
        const LocalSearchParameters& parameters)
{
    ...
    if (!data.parameters.timer.needs_to_end()) {
        ...
        // solutions, combining pairs of them with 'run_cross()' and locally
        // optimizing the result with 'run_kopt()' each generation.
        compute_edge_frequencies(data);
        while (true) {
            set_average_best(data);
            if (!data.output.solution.feasible()
                    || data.best_solution.length < data.output.solution.objective_value()) {
                data.algorithm_formatter.update_solution(
                        to_solution(data, data.best_solution),
