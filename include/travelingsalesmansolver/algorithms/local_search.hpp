...
struct LocalSearchData
{
    ...
    /** Scratch array used to build 'solution_to_tree()'. */
...
template <typename Distances>
void solution_to_tree(
        LocalSearchData<Distances>& data,
        const LocalSearchSolution& solution)
{
    data.tree_array[1] = 0;
    for (int i = 2; i <= data.number_of_vertices; ++i)
        data.tree_array[i] = solution.neighbors[data.tree_array[i - 1]][1];
...
template <typename Distances>
void tree_to_solution(
        const LocalSearchData<Distances>& data,
        LocalSearchSolution& solution)
{
    for (VertexId vertex_id = 0; vertex_id < data.number_of_vertices; ++vertex_id) {
        solution.neighbors[vertex_id][0] = previous_vertex(data, vertex_id);
        solution.neighbors[vertex_id][1] = next_vertex(data, vertex_id);
    }
...
void run_kopt(
        ...
        LocalSearchSolution& solution)
{
    solution_to_tree(data, solution);
    optimize(data);
    tree_to_solution(data, solution);
}
